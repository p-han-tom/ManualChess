...
func HedgeKnightPrimary(playerId string, casterId string, game *Match, targets []Position) error {
	...
	if math.Abs(float64(distanceToTarget)) != 1 {
		return fmt.Errorf("target is too far, distance must be exactly 1")
	}
